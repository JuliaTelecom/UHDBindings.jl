...
int main(int argc, char* argv[])
{
    ...
    size_t pointerChannel[2];
    size_t nbAntenna = 2;
    for (int i =0; i < nbAntenna; i++){
        pointerChannel[i] = i;
    }
    size_t channel = pointerChannel[0];
    ...
    uhd_stream_args_t stream_args = {
        ...
        .otw_format = "sc16",
        .args = "",
        .channel_list = pointerChannel,
        .n_channels = nbAntenna
    };
    ...
    /*EXECUTE_OR_GOTO(free_rx_streamer,*/
        err = uhd_usrp_get_rx_stream(usrp, &stream_args, rx_streamer);
        fprintf(stderr, "stream output is %d \n",err);
    /*)*/
        fprintf(stderr,"Configuration of streamer OK\n");
    ...
    buff = malloc(nbAntenna*samps_per_buff * 2 * sizeof(float));
    buffs_ptr = (void**)&buff;
